...
test('criar novo grupo de permissoes', async ({ assert, client }) => {
  ...
  const group = await Database.from('groups').where({ name: data.name, description: data.description })
...
test('edição de grupo de permissoe', async ({ assert, client }) => {
  ...
  const response = await client.put(`api/v1/group/${id}`).send(groupEdited).end()
